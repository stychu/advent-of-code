...
const mapPathToMoves = _.flow(
  _.A.map((direction: string) => moveMap.get(direction)),
  _.A.filter(_.G.isNotNullable),
  _.A.prepend({ x: 0, y: 0 })
);

const getHousesWithPresents = () =>
  _.A.reduce(
    { x: 0, y: 0, houses: {} },
    (
      acc: { x: number; y: number; houses: Record<string, number> },
      next: { x: number; y: number }
    ) => {
      acc.x += next.x;
      acc.y += next.y;

      const key = [acc.x, acc.y].toString();

      acc.houses[key] = (acc.houses[key] ?? 0) + 1;
...
const result = _.pipe(
  ...
  _.S.split(""),
  mapPathToMoves,
  getHousesWithPresents(),
  _.D.getUnsafe("houses"),
  _.D.values,
  _.A.length
);
...
const santaAndRoboSanta = _.pipe(
  data,
  _.S.split(""),
  _.A.reduceWithIndex(
    { santa: [], roboSanta: [] },
    (acc: { santa: string[]; roboSanta: string[] }, next, index) => {
      _.N.modulo(index, 2) === 0
        ? acc.santa.push(next)
        : acc.roboSanta.push(next);

      return acc;
    }
  ),
  _.D.map(mapPathToMoves),
  _.D.map((value) => getHousesWithPresents()(value)),
  _.D.map((value) => value.houses),
  _.D.values,
  _.A.reduce({}, (acc, next) => _.D.merge(acc)(next)),
  _.D.values,
  _.A.length
);

console.log(santaAndRoboSanta);
